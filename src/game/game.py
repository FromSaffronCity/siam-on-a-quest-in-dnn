...
class Game:
    ...
    def show_text(self):
        episode_txt = self.font.render(
            f"Episode: {self.episode_cnt}", True, (255, 255, 255))
        self.screen.blit(episode_txt, (10, 10))

        score_txt = self.font.render(
            f"Score: {self.level.player.score//5}", True, (255, 255, 255))
    ...
    def take_snapshot(self):
        ...
        display_matrix = display_matrix.reshape(
            (SHRINK_HEIGHT, SCREEN_HEIGHT // SHRINK_HEIGHT, SHRINK_HEIGHT, SCREEN_HEIGHT // SHRINK_HEIGHT, 3)).max(3).max(1)
